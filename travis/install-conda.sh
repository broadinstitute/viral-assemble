...
if [ -d "$MINICONDA_DIR" ] && [ -e "$MINICONDA_DIR/bin/conda" ]; then
    ...
    conda install -y -q java-jdk==8.0.112
    conda install -y -q conda-build # needed to build recipe
    conda install -y -q anaconda-client # needed to upload build package to anaconda.org
    conda install -y -q -c conda-forge -f curl # the bioconda curl is broken as of 21 Feb 2017
fi

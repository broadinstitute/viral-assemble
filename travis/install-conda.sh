...
if [ -d "$MINICONDA_DIR" ] && [ -e "$MINICONDA_DIR/bin/conda" ]; then
    ...
    travis_wait 30 conda install --quiet -y pycryptosat
    travis_wait 30 conda config --set sat_solver pycryptosat
fi

# update certs
travis_wait 1200 conda update -y openssl pyopenssl ca-certificates certifi

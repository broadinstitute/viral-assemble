...
def get_mpileup_allele_counts(inBam, chrom, pos, inConsFasta):
    ...
    SamtoolsTool().mpileup(inBam, pileupFileName, ['-r', '%s:%d-%d' % (chrom, pos, pos), '-d', '1000000000', '-L', '1000000000', '-Q', '13', '-f', inConsFasta])
                    ...
                    for row in util.file.read_tabfile(samp_to_isnv[sampleIDMatch(s)]):
                        ...
                        if row[0] == s_chrom:
                            ...
                            if naive_filter:
                                ...
                                # require allele frequency >= 5% for 1 library
                                # and >= 0.5% for >= 2 libraries
                                row['allele_counts'] = list((a, n) for a, n in row['allele_counts']
                                    if tot_n > 0 and ((row['n_libs'][a] == 1 and float(n) / tot_n >= 0.05) or
                                                      (row['n_libs'][a] >= 2 and float(n) / tot_n >= 0.005)))
